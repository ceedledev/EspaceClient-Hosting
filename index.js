...
db.on('error', err => {
  console.log(chalk.red("[DATABASE] Une erreur s'est produite lors de la tentative d'accès à la base de données."))
});
...
const listener = app.listen(settings.website.port, function() {
  console.log(chalk.green("[WEBSITE] Le site à bien démarrer " + listener.address().port + "."));
});
    ...
    if (cache > 0) {
      ...
      if (newsettings.api.client.ratelimits.enabled == true) {
        ...
        if (ipratelimit[ip] && ipratelimit[ip] >= newsettings.api.client.ratelimits.requests) {
          ...
          res.send(`<html><head><title>Votre trafic est limité.</title></head><body>Vous avez dépassé les limites de trafic.</body></html>`);
          return;
        }
  ...
  if (theme.settings.mustbeadmin.includes(req._parsedUrl.pathname)) {
    ...
    async function (err, str) {
      ...
      if (!req.session.userinfo || !req.session.pterodactyl) {
        if (err) {
          console.log(chalk.red(`[WEBSITE] Une erreur s'est produite sur le chemin ${req._parsedUrl.pathname}:`));
          console.log(err);
          return res.send("Une erreur s'est produite lors de la tentative de chargement de cette page. Veuillez contacter un administrateur pour résoudre ce problème.");
        };
        res.status(404);
        return res.send(str);
      };
      ...
      function (err, str) {
        ...
        delete req.session.password;
        if (err) {
          console.log(`[WEBSITE] Une erreur s'est produite sur le chemin ${req._parsedUrl.pathname}:`);
          console.log(err);
          return res.send("Une erreur s'est produite lors de la tentative de chargement de cette page. Veuillez contacter un administrateur pour résoudre ce problème.");
        };
        res.status(404);
